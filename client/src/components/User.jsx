...
function User(props) {
  ...
  useEffect(()=>{
    if (location.pathname === "/"){
      const fetchData = async function(){
        setIsLoading(true);
        await fetch(`${import.meta.env.VITE_REACT_APP_SERVER_URL}`+"/likedPost",{credentials: "include"}).then((res)=>{res.json().then((data)=>{setImage(data.image); setData(data);})
      });
      setIsLoading(false);
      }
      fetchData();
  ...
  return (
    ...
    <Avatar><img onClick={()=>{
      ...
      {isLoading?<h2>Loading...</h2>:<Grid container className="grid" spacing={3}>
            {data?.post.length!==0?
              data?.post.map((p) => {
                console.log(p.id);
                return (
              ...
              }):<h2 className="blog1">No liked blogs explore the new and trending section.</h2>}
          </Grid>}
      </Box>
    </>
  );
